...
## Дати й час — завжди тільки свіжі, ніколи з пам'яті
Вільні варіанти часу могли зайняти за лічені хвилини. Навіть якщо схожі дати/час вже згадувались раніше в цій розмові (у цьому чи попередньому сеансі) — ЗАБОРОНЕНО пропонувати або підтверджувати час, узятий з попереднього повідомлення. Перед КОЖНОЮ пропозицією часу викликай find_available_slots заново; перед КОЖНИМ підтвердженням конкретного часу — check_availability заново. Не покладайся на пам'ять розмови для дат.
...
4. Спитай ім'я пацієнта, ЯКЩО воно ще НЕ зустрічалося в цій розмові — перевір усю доступну історію, включно з попередніми сеансами. Якщо пацієнт уже називав ім'я раніше, навіть давно — використай його і не питай повторно.
5. Виклич find_available_slots з тривалістю обраної категорії. НЕ питай бажану дату/час першим. ЗАБОРОНЕНО самостійно перелічувати конкретні дати/час у тексті відповіді — вони вже показані пацієнту кнопками під повідомленням. Обмежся ОДНИМ коротким реченням типу "Ось найближчі вільні варіанти:" і жодних дат у тексті.
...
async function loadHistory(telegramId: number) {
  const { data, error } = await supabase
    ...
    .order('created_at', { ascending: false }) // спочатку найновіші...
    .limit(40); // ...щоб limit відрізав старі, а не нові
  if (error) {
    console.error('loadHistory error:', error.message);
    return [];
  }
  if (!data) return [];
  return data.reverse().map((m) => ({ role: m.role, parts: [{ text: m.content }] })); // повертаємо хронологічний порядок для Gemini
}
...
async function executeFunctionCall(
  ...
  if (fnName === 'find_available_slots') {
    ...
    return {
      response: {
        // Навмисно НЕ віддаємо модель iso/label по кожному слоту — маючи готові
        // читабельні дати під рукою, вона починала перелічувати їх текстом,
        // дублюючи клавіатуру. Компактна відповідь + пряма інструкція тут
        // прибирає спокусу.
        slots_found: slots.length,
        note: 'Варіанти вже показані пацієнту кнопками під цим повідомленням. НЕ перелічуй дати/час у тексті. Обмежся коротким "Ось найближчі вільні варіанти:" або подібною фразою.',
      },
      keyboard,
    };
  }
  ...
  try {
    ...
    if (isBusinessMessage && businessConnectionId) {
      ...
      if (conn && conn.owner_user_id === telegramId) {
        // ВАЖЛИВО: пауза прив'язується до ЧАТУ (пацієнта), а не до того, хто зараз
        // пише. Раніше тут стояло telegram_id: telegramId — а telegramId в цій гілці
        // це from.id ЛІКАРЯ (він же зараз відправник). Нижче, коли пише пацієнт,
        // telegramId — це вже ID ПАЦІЄНТА. Різні значення → рядок паузи ніколи не
        // знаходився при перевірці, і пауза фактично не діяла. chatId стабільний
        // для цієї розмови незалежно від того, хто в ній зараз пише.
        const pausedUntil = new Date(Date.now() + 25 * 60 * 1000).toISOString();
        await supabase
          .from('agent_pauses')
          .upsert({ business_connection_id: businessConnectionId, telegram_id: chatId, paused_until: pausedUntil });
